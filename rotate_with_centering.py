...
def path_to_svg_d_string(path):
    """
    Convert a svgpathtools Path object to an SVG 'd' attribute string
    using M, L, C, Q, A commands.
    """
    if len(path) == 0:
        return ""
    ...
    for seg in path:
        ...
        # Add MoveTo if this is the first segment or if there's a gap
        if current_pos is None or abs(start - current_pos) > 1e-6:
            d_parts.append(f"M {start.real},{start.imag}")
        
        if isinstance(seg, ToolsLine):
            d_parts.append(f"L {end.real},{end.imag}")
        
        elif isinstance(seg, ToolsCubic):
            c1, c2 = seg.control1, seg.control2
            d_parts.append(f"C {c1.real},{c1.imag} {c2.real},{c2.imag} {end.real},{end.imag}")
        
        elif isinstance(seg, ToolsQuad):
            c = seg.control
            d_parts.append(f"Q {c.real},{c.imag} {end.real},{end.imag}")
            ...
            d_parts.append(f"A {rx},{ry} {rotation} {large_arc},{sweep} {end.real},{end.imag}")
...
def save_paths_to_svg_svgwrite(paths, output_path, viewbox):
    """
    Save paths to SVG using svgwrite with the same format as step_to_svg.py.
    Uses 200x200 canvas with proper stroke attributes.
    """
    vx, vy, vw, vh = viewbox
    
    # Create SVG with same format as step_to_svg.py
    ...
    for path in paths:
        ...
        d_string = path_to_svg_d_string(path)
        if not d_string:
            continue
        
        # Create path with same attributes as step_to_svg.py
        svg_path = dwg.path(
            d=d_string,
            fill="none",
            stroke="#000000",
        ...
        svg_path.attribs['fill-rule'] = 'evenodd'
        svg_path.attribs['stroke-opacity'] = '1'
...
def create_rotated_file(input_path, output_path, angle_ccw):
    ...
    re-centers the content, and saves it using svgwrite format.
    """
    ...
    try:
        ...
        # Save using svgwrite with same format as step_to_svg.py
        save_paths_to_svg_svgwrite(rotated_paths, output_path, (vx, vy, vw, vh))
        return True
...
def process_folder(folder_path, output_npy_path):
    ...
    create_rotated_file(old_right, new_top, angle_ccw=180)
...
if __name__ == "__main__":
    # CONFIGURATION
    INPUT_DIR = r"C:\Users\LEGION\Desktop\cad_project\DeepCAD\data2\svg_raw\0000\00000119"
