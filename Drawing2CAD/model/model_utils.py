...
def _get_key_padding_mask_cad(commands, seq_dim=0):
    ...
    """
    with torch.no_grad():
        key_padding_mask = (commands == CAD_EOS_IDX).cumsum(dim=seq_dim) > 0
...
def _get_padding_mask_cad(commands, seq_dim=0, extended=False):
    with torch.no_grad():
        ...
        if extended:
            ...
            # Original code causes PyTorch memory issue with newer versions:
            # torch.narrow(padding_mask, seq_dim, 3, S-3).add_(torch.narrow(padding_mask, seq_dim, 0, S-3)).clamp_(max=1)
            # Fixed version to avoid in-place operations on shared memory:
            left_part = torch.narrow(padding_mask, seq_dim, 3, S-3)
            right_part = torch.narrow(padding_mask, seq_dim, 0, S-3)
            extended_part = (left_part + right_part).clamp(max=1)
            padding_mask.narrow(seq_dim, 3, S-3).copy_(extended_part)
...
def _get_group_mask(commands, seq_dim=0):
    ...
    with torch.no_grad():
        ...
        group_mask = (commands == CAD_EXT_IDX).cumsum(dim=seq_dim)
        return group_mask
...
def _get_visibility_mask(commands, seq_dim=0):
    ...
    S = commands.size(seq_dim)
    with torch.no_grad():
        visibility_mask = (commands == CAD_EOS_IDX).sum(dim=seq_dim) < S - 1
...
def _get_key_visibility_mask(commands, seq_dim=0):
    S = commands.size(seq_dim)
    with torch.no_grad():
        key_visibility_mask = (commands == CAD_EOS_IDX).sum(dim=seq_dim) >= S - 1
